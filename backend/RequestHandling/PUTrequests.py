...
def handle(TCP, path, data):
    ...
    if path.startswith(b'/users/'):
        ...
        if id.isnumeric():
            ...
            id_object = server.MyTCPHandler.userCollection.find_one({'id': int(id)})

            if id_object:
                query = {'id': int(id)}
                updated_info = {'$set': record}
                server.MyTCPHandler.userCollection.update_one(query, updated_info)

                user_info = server.MyTCPHandler.userCollection.find_one({'id': int(id)}, {'_id': False})
